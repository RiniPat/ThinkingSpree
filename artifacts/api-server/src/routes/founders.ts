...
router.patch("/founders/:id", async (req, res) => {
  ...
  const { name, email, companyName, sector, stage, description, acceleratorProgram, thinkingSheetUrl, incubatorId } = req.body;
  try {
    const [founder] = await db.update(foundersTable).set({ name, email, companyName, sector, stage, description, acceleratorProgram, thinkingSheetUrl, incubatorId: incubatorId ? parseInt(incubatorId) : undefined }).where(eq(foundersTable.id, id)).returning();
    if (!founder) { res.status(404).json({ error: "Not found" }); return; }
    res.json(mapFounder(founder));
  } catch (err) {
