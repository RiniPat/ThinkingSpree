...
import { useListSprints, useListIncubators } from "@workspace/api-client-react";
...
import {
  ...
  LayoutGrid, Table as TableIcon, ChevronUp, ChevronDown, RotateCcw,
} from "lucide-react";
            ...
            {sorted.map(s => {
              ...
              return (
                <tr key={s.id} onClick={() => onRowClick(s.id)}
                  ...
                  <td className="px-3 py-2.5 whitespace-nowrap text-foreground tabular-nums">
                  ...
                  <td className="px-3 py-2.5 text-foreground font-medium max-w-[160px] truncate" title={s.companyName}>{s.companyName}</td>
                  <td className="px-3 py-2.5 text-muted-foreground max-w-[120px] truncate" title={s.founderName}>{s.founderName}</td>
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full w-fit ${sc.bg} ${sc.color}`}>
