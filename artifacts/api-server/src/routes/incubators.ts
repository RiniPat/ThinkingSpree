...
/**
 * Allow-list: per product decision, the Summary surface now only shows
 * three program types: ISB, JU, Demo. Other types are filtered out at
 * the API layer.
 */
const ALLOWED_TYPES = ["isb", "ju", "demo"] as const;
  ...
  try {
    ...
    const result = await Promise.all(filtered.map(async (inc) => {
      const founders = await db
        ...
        .from(foundersTable)
        .where(eq(foundersTable.incubatorId, inc.id));
...
router.post("/incubators", async (req, res) => {
  ...
  if (!normalized) {
    res.status(400).json({ error: "type must be one of: isb, ju, demo" });
    return;
  }
  try {
    const [inc] = await db
...
router.get("/incubators/:id", async (req, res) => {
  ...
  try {
    ...
    const founders = await db
      .select()
      .from(foundersTable)
      .where(eq(foundersTable.incubatorId, id));
