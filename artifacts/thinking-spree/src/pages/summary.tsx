...
function AddIncubatorModal({ onClose }: { onClose: () => void }) {
  ...
  // Restricted to the three approved program types per product decision.
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-card border border-card-border rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h2 className="font-semibold text-foreground">Add Incubator / Program</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-1">Only ISB, JU, and Demo programs are tracked in the Summary view.</p>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Sheet URL</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2 border border-border rounded-md text-sm font-medium text-muted-foreground hover:bg-muted transition">Cancel</button>
...
export default function SummaryPage() {
  ...
  const typeLabels: Record<string, string> = { isb: "ISB", ju: "JU", demo: "Demo" };
