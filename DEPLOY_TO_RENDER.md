...
## Part 6 — Seed your data (10 min, runs locally)

The seed script reads your Excel files and writes them to the Neon database. It runs locally on your laptop, not on Render.

```bash
# From the project root on your laptop
cd scripts
mkdir -p seed-data
cp ~/Downloads/ISB_Summary_Sheet.xlsx   seed-data/
cp ~/Downloads/JU_Summary_Sheet.xlsx    seed-data/
cp ~/Downloads/Sheet_Tracking.xlsx      seed-data/

# Point at the same DATABASE_URL you put in Render
...
Expected output:
```
🌱 Seeding summary sheets…
✓ ISB incubator: id=1
✓ JU incubator:  id=2
✓ Demo incubator: id=3

📊 Importing ISB Summary Sheet…
  → 47 ISB ventures imported.

📊 Importing JU Summary Sheet…
  → 13 JU ventures imported.

📅 Importing Sheet Tracking…
  → 1931 sprints imported, 0 skipped

✅ Seed complete.
```

Refresh the app — dashboard, sprint tracking, and summary tabs all populated.

> 🔁 **Re-running is safe.** The script is idempotent — it finds existing founders/sprints by their natural keys and updates in place rather than duplicating. Run it any time the Excel files change.
...
| Build fails with `preinstall: Use pnpm instead` | The Replit-specific guard script in `package.json` is back. Confirm `package.json` has no `"preinstall"` key. |
| Sessions don't persist (logged out on every refresh) | The `connect-pg-simple` table didn't create. SSH into Render shell (or use the Neon SQL editor) and run: `SELECT * FROM user_sessions LIMIT 1;` If the table is missing, hit any API endpoint once — it auto-creates. |
| Seed script fails: "DATABASE_URL must be set" | You forgot to `export DATABASE_URL=...` in the same shell session before running `pnpm run seed:summary`. |
