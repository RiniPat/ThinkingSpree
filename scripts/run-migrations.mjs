...
async function shouldSeed(client) {
  try {
    const { rows } = await client.query("SELECT COUNT(*)::int AS c FROM founders");
    if (rows[0]?.c > 0) {
      console.log(`⏭  Found ${rows[0].c} founders in DB — skipping seed`);
      return false;
    }
  } catch (err) {
    console.log("⏭  Cannot read founders table — skipping seed:", err.message);
    return false;
  }
